...
class CSFTZBankCharges(Document):
	...
	def on_submit(self):
		...
		for detail in self.get("csf_tz_bank_charges_detail"):
			pe_details = self.create_pe(detail.debit_amount, detail.value_date, detail.reference_number)

			detail.ref_docname = pe_details["reference_name"]
			detail.ref_doctype = pe_details["reference_type"]
			self.flags.payments.append(pe_details)
	...
	def payment_reconcile(self):
		try:
			...
			reconcile.invoices = []
			for invoice in self.flags.invoices:
			...
			reconcile.payments = []
			for payment in self.flags.payments:
			...
			reconcile.allocate_entries({"invoices": self.flags.invoices, "payments": self.flags.payments})

			reconcile.reconcile()
		except Exception as e:
			frappe.throw(_("An error occurred during payment reconciliation: {0}").format(str(e)))
