...
class ReceivablePayableReport:
	...
	def run(self, args):
		...
		self.set_defaults()
		self.party_naming_by = frappe.db.get_single_value(*args.get("naming_by"))
		self.get_columns()
		self.get_data()
		self.get_chart_data()
	...
	def update_voucher_balance(self, gle):
		...
		if row.account_currency != self.company_currency:
			if row.voucher_type in ["Sales Invoice", "Purchase Invoice"]:
				row.foreign_currency, row.foreign_amount = (
					frappe.get_value(row.voucher_type, row.voucher_no, ["currency", "rounded_total"]) or ""
				)
	...
	def build_delivery_note_map(self):
		if self.invoices and self.filters.show_delivery_notes:
			...
			""".format(",".join(["%s"] * len(self.invoices))),
				tuple(self.invoices),
				as_dict=1,
			)
			...
			dn_against_si = frappe.db.sql(
				...
				where against_sales_invoice in ({})
			""".format(",".join(["%s"] * len(self.invoices))),
				tuple(self.invoices),
				as_dict=1,
			)
	...
	def get_future_payments_from_journal_entry(self):
		...
		return frappe.db.sql(
			...
			select
				...
				sum({amount_field}) as future_amount,
	...
	def get_gl_entries(self):
		...
		self.gl_entries = frappe.db.sql(
			...
			where
				...
				{date_condition} {conditions} {order_by}""",
			values,
			as_dict=True,
		)
	...
	def get_hierarchical_filters(self, doctype, key):
		...
		return f"""party in (select name from tabCustomer
			where exists(select name from `tab{doctype}` where lft >= {lft} and rgt <= {rgt}
				and name=tabCustomer.{key}))"""
		...
		if accounting_dimensions:
			for dimension in accounting_dimensions:
				if self.filters.get(dimension.fieldname):
					...
					conditions.append(f"{dimension.fieldname} in %s")
	...
	def get_columns(self):
		...
		self.add_column(
			label=_(self.party_type), fieldname="party", fieldtype="Link", options=self.party_type, width=180
		)
	...
	def get_chart_data(self):
		...
		self.chart = {"data": {"labels": self.ageing_column_labels, "datasets": rows}, "type": "percentage"}
