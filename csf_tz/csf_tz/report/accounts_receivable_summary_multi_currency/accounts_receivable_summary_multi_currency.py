...
from csf_tz.csf_tz.report.accounts_receivable_summary_multi_currency.accounts_receivable_utils import (
...
class AccountsReceivableSummary(ReceivablePayableReport):
	...
	def get_data(self, party_naming_by, args):
		...
		partywise_advance_amount = (
			get_partywise_advanced_payment_amount([args.get("party_type")], self.filters.get("report_date"))
			or {}
		)
		for party, party_dict in iteritems(partywise_total):
			...
			row += [
				party_dict.invoiced_amt,
				paid_amt,
				party_dict.credit_amt,
				party_dict.outstanding_amt,
				party_dict.range1,
				party_dict.range2,
				party_dict.range3,
				party_dict.range4,
			...
			if args.get("party_type") == "Customer":
				row += [
					self.get_territory(party),
					self.get_customer_group(party),
					", ".join(set(party_dict.sales_person)),
				]
			if args.get("party_type") == "Supplier":
	...
	def make_data_dict(cols, data):
		data_dict = []
		for d in data:
			data_dict.append(frappe._dict(zip(cols, d, strict=False)))
