...
class ReceivablePayableReport:
	...
	def run(self, args):
		party_naming_by = frappe.db.get_single_value(*args.get("naming_by"))
	...
	def get_columns(self, party_naming_by, args):
		...
		columns.append(
			{"label": _("Voucher Type"), "fieldtype": "Data", "fieldname": "voucher_type", "width": 110}
		)
		...
		if self.filters.based_on_payment_terms:
			columns.append(
				{"label": _("Payment Term"), "fieldname": "payment_term", "fieldtype": "Data", "width": 120}
			)
			columns.append(
				{
		...
		if "range1" not in self.filters:
			self.filters["range1"] = "30"
		if "range2" not in self.filters:
			self.filters["range2"] = "60"
		if "range3" not in self.filters:
			self.filters["range3"] = "90"
		if "range4" not in self.filters:
		...
		columns += [
			...
			{"fieldname": "pdc/lc_ref", "label": _("PDC/LC Ref"), "fieldtype": "Data", "width": 110},
			{
					...
					if self.filters.based_on_payment_terms and self.payment_term_map.get(gle.voucher_no):
						...
						if credit_note_amount:
							row = self.prepare_row_without_payment_terms(
								party_naming_by, args, gle, temp_outstanding_amt, temp_credit_note_amt
							)
							data.append(row)

					else:
						row = self.prepare_row_without_payment_terms(
							party_naming_by, args, gle, outstanding_amount, credit_note_amount
						)
						data.append(row)
		return data
	...
	def prepare_row_without_payment_terms(
		self, party_naming_by, args, gle, outstanding_amount, credit_note_amount
	):
		pdc_list = self.pdc_details.get((gle.voucher_no, gle.party), [])
		pdc_amount = 0
		pdc_details = []
	...
	def prepare_row(
		...
		row += get_ageing_data(
			cint(self.filters.range1),
			cint(self.filters.range2),
			cint(self.filters.range3),
			cint(self.filters.range4),
			self.age_as_on,
			entry_date,
			outstanding_amount,
		)
		...
		if self.filters.ageing_based_on == "Supplier Invoice Date" and getdate(bill_date) > getdate(
			self.filters.report_date
		):
			row[-1] = row[-2] = row[-3] = row[-4] = row[-5] = 0
		...
		return list(
			set(
				[
					...
					for e in self.get_gl_entries(party_type, report_date, for_future=True)
				]
			)
		)
	...
	def is_receivable_or_payable(gle, dr_or_cr, future_vouchers, return_entries):
		return (
			...
			(gle.against_voucher_type in ["Sales Order", "Purchase Order"])
			or
			...
			(
				...
				and gle.voucher_no in return_entries
				and not return_entries.get(gle.voucher_no)
			)
			or
	...
	def get_outstanding_amount(self, gle, report_date, dr_or_cr, return_entries):
		...
		for e in self.get_gl_entries_for(gle.party, gle.party_type, gle.voucher_type, gle.voucher_no):
			if getdate(e.posting_date) <= report_date and (
				e.name != gle.name
				or (e.voucher_no in return_entries and not return_entries.get(e.voucher_no))
			):
				amount = flt(e.get(reverse_dr_or_cr), self.currency_precision) - flt(
					e.get(dr_or_cr), self.currency_precision
				)
		...
		voucher_amount = flt(gle.get(dr_or_cr), self.currency_precision) - flt(
			gle.get(reverse_dr_or_cr), self.currency_precision
		)
		if gle.voucher_no in return_entries and not return_entries.get(gle.voucher_no):
			voucher_amount = 0

		outstanding_amount = flt(
			(voucher_amount - payment_amount - credit_note_amount), self.currency_precision
		)
	...
	def get_party_name(self, party_type, party_name):
		return (
			self.get_party_map(party_type)
			.get(party_name, {})
			.get("customer_name" if party_type == "Customer" else "supplier_name")
			or ""
		)
	...
	def get_gl_entries(self, party_type, date=None, for_future=False):
		...
		if date and not for_future:
			conditions += f" and posting_date <= '{date}'"
		...
		self.gl_entries = frappe.db.sql(
			...
			select
				...
				against_voucher_type, against_voucher, account_currency, remarks, {select_fields}
			from
				`tabGL Entry`
			where
				docstatus < 2 and party_type=%s and (party is not null and party != '') {conditions}
		...
		if not hasattr(self, "gl_entries_map"):
			...
			for gle in self.get_gl_entries(party_type):
				if gle.against_voucher_type and gle.against_voucher:
					self.gl_entries_map.setdefault(gle.party, {}).setdefault(
						gle.against_voucher_type, {}
					).setdefault(gle.against_voucher, []).append(gle)

		return self.gl_entries_map.get(party, {}).get(against_voucher_type, {}).get(against_voucher, [])
	...
	def get_payment_term_detail(self, voucher_nos):
		...
		payment_terms_details = frappe.db.sql(
			...
			si.docstatus = 1 and si.company = '{}' and
			si.name in ({}) order by ps.due_date
		""".format(frappe.db.escape(self.filters.company), ",".join(["%s"] * len(voucher_nos))),
			(tuple(voucher_nos)),
			as_dict=1,
		)
		...
		for d in payment_terms_details:
			...
			payment_term_map.setdefault(d.name, []).append(
				frappe._dict(
					{
						"due_date": d.due_date,
	...
	def get_chart_data(self, columns, data):
		...
		rows = []
		for d in data:
			rows.append({"values": d[self.ageing_col_idx_start : self.ageing_col_idx_start + 5]})

		return {
			"data": {"labels": [d.get("label") for d in ageing_columns], "datasets": rows},
			"type": "percentage",
		}
...
def get_ageing_data(
	first_range, second_range, third_range, fourth_range, age_as_on, entry_date, outstanding_amount
):
...
def get_pdc_details(party_type, report_date):
	...
	if scrub(party_type):
		amount_field = (
			"jea.debit_in_account_currency" if party_type == "Supplier" else "jea.credit_in_account_currency"
		)
	else:
...
def get_voucher_details(party_type, voucher_nos, dn_details):
	...
	if party_type == "Customer":
		for si in frappe.db.sql(
			...
			where inv.docstatus=1 and inv.name in ({})
			group by inv.name
			""".format(",".join(["%s"] * len(voucher_nos))),
			(tuple(voucher_nos)),
			as_dict=1,
	...
	if party_type == "Supplier":
		for pi in frappe.db.sql(
			...
			from `tabPurchase Invoice` where docstatus = 1 and name in ({})
			""".format(",".join(["%s"] * len(voucher_nos))),
			(tuple(voucher_nos)),
			as_dict=1,
		):
	...
	for pi in frappe.db.sql(
		"""select name, due_date, bill_no, bill_date from
		`tabJournal Entry` where docstatus = 1 and bill_no is not NULL and name in ({})
		""".format(",".join(["%s"] * len(voucher_nos))),
		(tuple(voucher_nos)),
		as_dict=1,
	):
