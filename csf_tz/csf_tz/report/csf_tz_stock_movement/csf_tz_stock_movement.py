...
def execute(filters=None):
	...
	for row in sl_entries:
		...
		] = row["purchase_qty"] = row["purchase_value"] = row["sold_qty"] = row["sold_value"] = row[
			"adjustment_qty"
		] = row["adjustment_value"] = row["consumed_qty"] = row["consumed_value"] = row["produced_qty"] = row[
			"produced_value"
		] = row["received_qty"] = row["received_value"] = row["issued_qty"] = row["issued_value"] = 0

		if not any(
			record["item_code"] == row.item_code and record["warehouse"] == row.warehouse
			for record in opening_balance_item_wise
		):
			opening_balance_item_wise.append({"item_code": row.item_code, "warehouse": row.warehouse})
			opening_row = get_opening_balance(filters, row.item_code, row.warehouse, sl_entries)

			if opening_row:
				row["opening_qty"] = opening_row["qty_after_transaction"]
				row["opening_value"] = flt(row["opening_qty"]) * opening_row["valuation_rate"]

		if row.voucher_type == "Stock Reconciliation":
			row["reconciliation_qty"] = row.stock_value_difference / (row.valuation_rate or 1)

			row["reconciliation_value"] = row.stock_value_difference

		if row.voucher_type == "Purchase Receipt" or row.voucher_type == "Purchase Invoice":
		...
		if row.voucher_type == "Stock Entry":
			...
			stock_entry_type = frappe.db.get_value(row.voucher_type, row.voucher_no, "stock_entry_type")
...
def prepare_data(data):
	...
	grouped_data = {key: list(group) for key, group in groupby(sorted_data, key=itemgetter("item_code"))}
	result = []
		...
		result.append(
			{
				...
				"reconciliation_qty": sum(entry["reconciliation_qty"] for entry in group),
				"reconciliation_value": sum(entry["reconciliation_value"] for entry in group),
