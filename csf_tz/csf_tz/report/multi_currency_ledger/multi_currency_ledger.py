	...
	for entry in gl_entries:
		...
		if docktype == "Payment Entry":
			...
			if entry["party"]:
				...
				for item in items:
					items_list += f"({item.item_name}"
					if item.service_start_date:
						items_list += f" , {item.service_start_date} "
					if item.service_end_date:
						items_list += f", {item.service_end_date}"
					items_list += ") "
...
def get_accountwise_gle(filters, gl_entries, gle_map):
	...
	for _key, value in consolidated_gle.items():
...
def get_result_as_list(data, filters):
	...
	for d in data:
		...
		if not (
			(d.get("credit_foreign") and d.get("debit_foreign"))
			and (d.get("credit_foreign") == d.get("debit_foreign"))
		):
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _(f"Debit ({currency})"), "fieldname": "debit", "fieldtype": "Float", "width": 100},
		{"label": _(f"Credit ({currency})"), "fieldname": "credit", "fieldtype": "Float", "width": 100},
		{"label": _("Debit Foreign"), "fieldname": "debit_foreign", "fieldtype": "Float", "width": 100},
		{"label": _("Credit Foreign"), "fieldname": "credit_foreign", "fieldtype": "Float", "width": 100},
		{
		...
		{"label": _(f"Balance ({currency})"), "fieldname": "balance", "fieldtype": "Float", "width": 130},
	]
	...
	columns.extend(
		[
			...
			{"label": _("Against AC Name"), "fieldname": "against_acount", "width": 120},
			{"label": _("Against Debit"), "fieldname": "against_debit", "fieldtype": "Float", "width": 100},
			{"label": _("Against Credit"), "fieldname": "against_credit", "fieldtype": "Float", "width": 100},
			{
			...
			{"label": _("Items"), "fieldname": "items", "width": 400},
		]
	)
