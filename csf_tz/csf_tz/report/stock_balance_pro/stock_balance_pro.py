...
def get_columns(filters):
	...
	if filters.get("show_variant_attributes"):
		columns += [
			{"label": att_name, "fieldname": att_name, "width": 100} for att_name in get_variants_attributes()
		]
...
def get_conditions(filters):
	...
	if filters.get("to_date"):
		conditions += " and sle.posting_date <= {}".format(frappe.db.escape(filters.get("to_date")))
	else:
		frappe.throw(_("'To Date' is required"))

	if filters.get("company"):
		conditions += " and sle.company = {}".format(frappe.db.escape(filters.get("company")))
	...
	if filters.get("warehouse"):
		...
		if warehouse_details:
			conditions += f" and exists (select name from `tabWarehouse` wh \
                where wh.lft >= {warehouse_details.lft} and wh.rgt <= {warehouse_details.rgt} and sle.warehouse = wh.name)"

	if filters.get("warehouse_type") and not filters.get("warehouse"):
		conditions += " and exists (select name from `tabWarehouse` wh \
            where wh.warehouse_type = '{}' and sle.warehouse = wh.name)".format(filters.get("warehouse_type"))
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql(
        ...
        where sle.is_cancelled = 0
            ...
            and sle.docstatus < 2 {item_conditions_sql} {conditions}
        UNION ALL
        select
            sle.item_code, sle.warehouse, sle.posting_date, sle.actual_qty, sle.valuation_rate,
            sle.company, sle.voucher_type, sle.qty_after_transaction, sle.stock_value_difference,
	...
	for d in sle:
		...
		if key not in iwb_map:
			iwb_map[key] = frappe._dict(
				{"opening_qty": 0.0, "in_qty": 0.0, "out_qty": 0.0, "excise_stock": 0.0, "bal_qty": 0.0}
			)
...
def get_item_details(items, sle, filters):
	...
	if filters.get("include_uom"):
		...
		cf_join = "left join `tabUOM Conversion Detail` ucd on ucd.parent=item.name and ucd.uom={}".format(
	...
	res = frappe.db.sql(
        ...
        where
            item.name in ({})
    """.format(cf_field, cf_join, ",".join(["%s"] * len(items))),
		items,
		as_dict=1,
	)
...
def get_variant_values_for(items):
	...
	for attr in frappe.db.sql(
        ...
        from `tabItem Variant Attribute` where parent in ({})
