...
def get_rootwise_opening_balances(filters, report_type):
	...
	if filters.cost_center:
		...
		additional_conditions += f""" and cost_center in (select name from `tabCost Center`
	...
	if accounting_dimensions:
		for dimension in accounting_dimensions:
			if filters.get(dimension):
				additional_conditions += f""" and {dimension} in (%({dimension})s) """
...
def prepare_data(accounts, filters, total_row, parent_children_map, company_currency):
	data = []
	currency_value = frappe.get_list(
		"Currency Exchange",
		filters={"from_currency": "USD", "to_currency": "TZS"},
	...
	for d in accounts:
		...
		for key in value_fields:
			row[key] = flt(d.get(key, 0.0) / currency_value[len(currency_value) - 1].exchange_rate, 3)
