...
def generate_additional_salary_records():
	...
	if len(additional_salary_list) > 0:
		for entry in additional_salary_list:
			if entry.last_transaction_date is None:
			...
			if next_date <= add_days(getdate(today_date), 13):
				...
				additional_salary.overwrite_salary_structure_amount = entry.overwrite_salary_structure_amount
...
def get_employee_base_salary_in_hours(employee, payroll_date):
	...
	working_hours_per_month = frappe.db.get_single_value("CSF TZ Settings", "working_hours_per_month")
	if not working_hours_per_month:
		frappe.throw(
			_("Working Hours per Month not defind in CSF TZ Settings. Define it there and try again.")
		)
...
def set_employee_base_salary_in_hours(doc, method):
	if doc.based_on_hourly_rate:
		...
		base_salary_in_hours = get_employee_base_salary_in_hours(doc.employee, doc.payroll_date)[
