...
def validate(doc, method):
	...
	shift_timings = get_employee_shift_timings(doc.employee, get_datetime(doc.time), True)
...
def get_employee_shift_timings(
	...
	prev_shift = get_employee_shift(
		employee,
		(curr_shift.start_datetime if curr_shift else for_timestamp) + timedelta(days=-1),
		consider_default_shift,
		"reverse",
	)
