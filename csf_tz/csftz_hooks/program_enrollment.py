...
def get_fee_schedule(program, academic_year, academic_term=None, student_category=None):
	...
	fees_list = []
	for i in fs:
		fs_academic_year = frappe.get_value("Fee Structure", i["fee_structure"], "academic_year") or ""
		fs_academic_term = "False"
		if academic_term:
			fs_academic_term = frappe.get_value("Fee Structure", i["fee_structure"], "academic_term") or ""
