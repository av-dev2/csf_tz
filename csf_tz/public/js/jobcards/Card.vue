		...
		<v-dialog v-model="Dialog" max-width="900">
			<v-card class="px-3">
				<v-card-title class="mt-2">
					<span class="headline indigo--text">{{ cardData.operation.name }}</span>
					<v-spacer></v-spacer>
					<div
						class="stopwatch"
						style="
...
export default {
	...
	methods: {
		...
		start_job() {
			let row = frappe.model.add_child(this.cardData, "Job Card Time Log", "time_logs");
		...
		save(action = "Save") {
			...
			this.members.forEach((element) => {
				let employee_name = this.employees.find((emp) => emp.name == element).employee_name || "";
