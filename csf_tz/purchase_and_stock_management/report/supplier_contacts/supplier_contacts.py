	...
	for party_detail in map(list, party_details):
		...
		if not any([addresses, contacts]):
			...
			for idx in range(0, max_length):
				...
				address = (
					addresses[idx] if idx < len(addresses) else ["" for field in field_map.get("Address", [])]
				)
				contact = (
					contacts[idx] if idx < len(contacts) else ["" for field in field_map.get("Contact", [])]
				)
...
def get_default_address_contact_filters(party_type, docname):
	return [["Dynamic Link", "link_doctype", "=", party_type], ["Dynamic Link", "link_name", "=", docname]]
