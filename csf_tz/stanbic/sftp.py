...
def get_local_path(folders_name=None):
...
def download_stanbank_files(settings_name):
	...
	paramiko_obj = Paramiko(settings.sftp_url, settings.sftp_user, key_file_path, settings.port)
		...
		try:
			if file.endswith(".xml"):
				if "ACK" in file or "INTAUD" in file or "FINAUD" in file:
					path = get_local_path(["private", "files", "stanbic", "inbox", file])

					file_dict = parse_xml(path)
					file_json = frappe.as_json(file_dict)
					print("filename", path)
					pain_doc_name = file_dict["Document"]["CstmrPmtStsRpt"]["OrgnlGrpInfAndSts"]["OrgnlMsgId"]
					pain_doc = frappe.get_doc("Stanbic Payments Initiation", pain_doc_name)
