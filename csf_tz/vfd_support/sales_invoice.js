...
function show_vfd_preview_dialog(frm, payload, vfd_provider) {
	...
	function buildInfoRow(label, value) {
		...
		if (!hasVal) return "";
		return `<div class="vfd-row"><span class="vfd-label">${frappe.utils.escape_html(
			label
		)}</span><span class="vfd-value">${frappe.utils.escape_html(String(value))}</span></div>`;
	}
  ...
  <div class="vfd-preview-root">
    ...
    <div class="vfd-center" style="margin-bottom:4px;">
      ...
      <div class="vfd-muted" style="font-size:11.5px;">TIN: ${frappe.utils.escape_html(
			frm.doc.tax_id || "-"
		)}&nbsp;&nbsp;|&nbsp;&nbsp;RECEIPT DATE: ${frappe.utils.escape_html(receipt_date)}</div>
    </div>
    <hr class="vfd-hr" />
